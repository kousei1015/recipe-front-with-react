...
バックエンド(Ruby on Railsで開発しました)に関するコードは　https://github.com/kousei1015/recipe-backend　こちらのレポジトリにあります
...
- S3(ビルドした静的ファイルをホスティングするために使用)
- CloudFront(HTTPS通信とコンテンツ配信の高速化のために使用)
- Route53(独自ドメインを使うために使用)

この3つのサービスを使って、デプロイしました。
...
にも書きましたが、
バックエンドのインフラについては、こちらにも書いていますが、Dockerを使ってローカルの開発環境を構築していたので、AWSのコンテナサービスを使いたいと思い、今回はLightsailのコンテナサービスを利用しました。以前は、[こちらのレポジトリ](https://github.com/kousei1015/map-collection)をFargateでデプロイした経験もあり、他にもECS on EC2も選択肢にありましたが、他のサービスを調べていく内にLightsailもコンテナをサポートし、料金も月7ドルからと安価であること、そしてデプロイが非常に簡単であることを知り、今回はLightsailを採用しました。Lightsailのコンテナサービスを使ったデプロイに関しては情報が少ないと感じましたが、ECSのデプロイ経験があれば、簡単にデプロイできると思います。また、データベースについてもLightsailのサービスを利用しており、比較的安価でコストパフォーマンスが良いことから採用を決めました。
